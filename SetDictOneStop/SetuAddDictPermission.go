...
%s`, e.Policyname, e.Starrocks, e.Catalog, e.User, model, strings.Join(tablemsg, "\n"))
		...
		//给userid发送一个飞书信息
		go func() {
			appName, ok1 := util.MetaConf["slow_query_lark_app"].(string)
			appid, ok2 := util.MetaConf["slow_query_lark_appid"].(string)
			appSecret, ok3 := util.MetaConf["slow_query_lark_appsecret"].(string)
			larkproxy, ok4 := util.MetaConf["slow_query_proxy_feishu"].(string)
			if !ok1 || !ok2 || !ok3 || !ok4 ||
				appName == "" || appid == "" || appSecret == "" || larkproxy == "" {
				return
			}
			...
			var username string
			meta := lark.Lark2userid(submitUser)
			if meta != nil {
			...
			err := lark.Send2userid(submitUser, message)
			if err != nil {
				util.Loggrs.Warn(client, err.Error())
				return
			}
			if util.Read.Schema.Lkremind != nil {
				go lark.Send2Markdown(fmt.Sprintf("%s向%s发送了一条信息", time.Now().Format("2006-01-02 15:04:05"), submitUser), message, util.Read.Schema.Lkremind)
			}
		}()
