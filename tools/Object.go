...
// Exec 执行授权ddl
func Exec(s *Schem, sql string) string {
	var dict string
	...
	if err != nil {
		return fmt.Sprintf("Fail %s %s: CATALOG=%s,USER=%s,TABLE=%s,PERMISSION=%s %s", dict, s.App, s.Catalog, s.User, s.Table, s.Permission, err.Error())
	}
	return fmt.Sprintf("Ok   %s %s: CATALOG=%s,USER=%s,TABLE=%s,PERMISSION=%s", dict, s.App, s.Catalog, s.User, s.Table, s.Permission)
}

// OnExec20 执行授权ddl
func OnExec20(s *Schem, sql string) util.SetResult {
	var dict string
	if strings.Contains(strings.ToLower(sql), "grant") {
		dict = "授予"
	}
...
// Execute 执行授权ddl
func Execute(mutex *sync.Mutex, s *Schem, sql string) string {
	mutex.Lock()
	defer mutex.Unlock()
	var dict string
	if strings.Contains(strings.ToLower(sql), "grant") {
...
// Onexec 执行授权ddl
func Onexec(mutex *sync.Mutex, s *Schem, sql string) util.SetResult {
	mutex.Lock()
	defer mutex.Unlock()
	var dict string
	if strings.Contains(strings.ToLower(sql), "grant") {
...
// ExecRole 执行授权ddl
func ExecRole(s SchemRole, sql string) string {
	var dict string
	if strings.Contains(strings.ToLower(sql), "grant") {
		dict = "授予"
	}
	...
	r := s.Db.Exec(stmt)
	if r.Error != nil {
		return fmt.Sprintf("Fail %s %s: CATALOG=%s,ROLE=%s,TABLE=%s,PERMISSION=%s %s", dict, s.App, s.Catalog, s.Role, s.Table, s.Permission, r.Error.Error())
	}
	return fmt.Sprintf("Ok   %s %s: CATALOG=%s,ROLE=%s,TABLE=%s,PERMISSION=%s", dict, s.App, s.Catalog, s.Role, s.Table, s.Permission)
}

// OnExecRole 执行授权ddl
func OnExecRole(s SchemRole, sql string) util.SetRole {
	stmt := fmt.Sprintf(`set @policyName="%s";%s`, s.Policy, sql)
	fmt.Println(stmt)
	r := s.Db.Exec(stmt)
	if r.Error != nil {
...
// OnExecAdd 执行账号管理ddl
func OnExecAdd(s Dicts, sql string) util.SetUserId {
	/*匹配授权，回收标识*/
	stmt := fmt.Sprintf(`set @policyName="%s";%s`, s.Policy, sql)
...
// ExecAdd 执行账号管理ddl
func ExecAdd(s Dicts, sql string) string {
	/*匹配授权，回收标识*/
	var dict string
	if s.Option == "create" {
		dict = "创建账号"
	...
	r := s.Db.Exec(stmt)
	if r.Error != nil {
		return fmt.Sprintf("Fail %s %s: USER=%s,ACTION=%s %v", dict, s.App, s.User, s.Option, r.Error.Error())
	}
	return fmt.Sprintf("Ok   %s %s: USER=%s,ACTION=%s %v", dict, s.App, s.User, s.Option, "")
}

// ExecDatabase 执行管理数据库
func ExecDatabase(s DictDB, sql string) string {
	/*匹配授权，回收标识*/
	var dict string
	if strings.Contains(strings.ToLower(sql), "create") {
		dict = "创建库"
	}
	if strings.Contains(strings.ToLower(sql), "drop") {
		dict = "销毁库"
	}
	if strings.Contains(strings.ToLower(sql), "alter") {
		dict = "重置库容量大小"
	}
	...
	r := s.Db.Exec(stmt)
	if r.Error != nil {
		return fmt.Sprintf("Fail %s %s: USER=%s,DATABASE=%s,ACTION=%s %v", dict, s.App, s.User, s.Database, s.Option, r.Error.Error())
	}
	return fmt.Sprintf("Ok   %s %s: USER=%s,DATABASE=%s,ACTION=%s %v", dict, s.App, s.User, s.Database, s.Option, "")
}

// OnExecDatabase 执行管理数据库
func OnExecDatabase(s DictDB, sql string) util.SetDataBase {
	stmt := fmt.Sprintf(`set @policyName="%s";%s`, s.Policy, sql)
	util.Loggrs.Info(stmt)
