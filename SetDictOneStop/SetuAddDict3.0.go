...
func SetuAddDict3(t *dictStruct, once *sync.Once) []util.SetResult {
	...
	if strings.Contains(t.table, ".*") {
		for _, p := range strings.Split(t.e.Permission, ",") {
			...
			case "SELECT":
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON TABLE %s %s %s`, t.e.Catalog, t.dictH, p, t.table, t.dictE, t.user), "Table"))
				if t.e.Catalog == "default_catalog" {
					tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf("SET CATALOG %s;%s %s ON ALL VIEWS IN DATABASE %s %s USER %s", t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "View"))
					tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf("SET CATALOG %s;%s %s ON ALL MATERIALIZED VIEWS IN DATABASE %s %s USER %s", t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "MView"))
				}
			case "INSERT", "EXPORT", "UPDATE", "DELETE", "ALTER", "DROP":
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON ALL TABLES IN DATABASE %s %s %s`, t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "Database"))
				if t.e.Catalog == "default_catalog" {
					if strings.ToUpper(p) == "DROP" || strings.ToUpper(p) == "ALTER" {
						tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf("SET CATALOG %s;%s %s ON ALL VIEWS IN DATABASE %s %s USER %s", t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "Database"))
						tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf("SET CATALOG %s;%s %s ON ALL MATERIALIZED VIEWS IN DATABASE %s %s USER %s", t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "Database"))
					}
				}
			case "CREATE TABLE", "CREATE VIEW", "CREATE FUNCTION", "CREATE MATERIALIZED VIEW", "CREATE ROW ACCESS POLICY", "CREATE MASKING POLICY":
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON DATABASE %s %s %s`, t.e.Catalog, t.dictH, p, strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "Database"))

			case "CREATE_TABLE", "CREATE_VIEW", "CREATE_FUNCTION", "CREATE_MATERIALIZED_VIEW", "CREATE_ROW_ACCESS_POLICY", "CREATE_MASKING_POLICY":
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON DATABASE %s %s %s`, t.e.Catalog, t.dictH, strings.ReplaceAll(p, "_", " "), strings.ReplaceAll(t.table, ".*", ""), t.dictE, t.user), "Database"))
		...
		case "VIEW":
			...
			if mv {
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON MATERIALIZED VIEW %s %s %s`, t.e.Catalog, t.dictH, t.e.Permission, t.table, t.dictE, t.user), "MView"))
			} else {
				tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON VIEW %s %s %s`, t.e.Catalog, t.dictH, t.e.Permission, t.table, t.dictE, t.user), "View"))
			}
		case "BASE TABLE":
			tableResult = append(tableResult, tools.Onexec(&mutex, &schem, fmt.Sprintf(`SET CATALOG %s;%s %s ON TABLE %s %s %s`, t.e.Catalog, t.dictH, t.e.Permission, t.table, t.dictE, t.user), "Table"))
