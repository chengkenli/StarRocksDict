...
`20260616:加固mysql配置表password字段，存储的数据必须是密文，否测StarRocks登录时解密会失败。`
...
CREATE TABLE starrocks_information_connections (
  ...
  status int NOT NULL DEFAULT '0' COMMENT '是否启用该配置,0 off, 1 on',
...
`INSERT INTO db.starrocks_information_connections (app, nickname, alias, feip, user, password, feport, status) VALUES('sr-ranger', 'StarRocks(Tencent Cloud) SR-RangerQa', NULL, 'FE连接地址', 'root', '密码', 9030, 1);`
---
