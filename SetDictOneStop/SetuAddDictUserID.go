...
func SetuAddUserID(c *gin.Context) {
	...
	util.Loggrs.Info(client, "账户管理")

	data, err := c.GetRawData()
	util.Loggrs.Info(client, string(data))
	...
	util.Loggrs.Info(client, fmt.Sprintf("%0.2f", version))
	...
	for _, user := range strings.Split(add.User, ",") {
		...
		go func(user string) {
			...
			r := db.Raw("show grants for " + user).Scan(&m)
			if r.Error != nil {
				util.Loggrs.Warn(client, r.Error.Error())
			...
			go func() {
				if util.MetaConf["slow_query_email_from"] == nil {
					return
				}
				...
				util.Loggrs.Info(client, fmt.Sprintf("#发送邮件 %s", to))
				Send.SendEmail2(
					&util.Emailinfo{
				...
				util.Loggrs.Info(client, "send success.")
			}()
...
账号：(%s)%s
%s
`, u.App, u.State, u.User, strings.Join(util.Read.Schema.Connects, "\n"))
				} else {
					message = fmt.Sprintf(`
单号：%s
集群：%s
账号：(%s)%s
%s
`, u.Policyname, u.State, u.App, u.User, strings.Join(util.Read.Schema.Connects, "\n"))
				}

				messages := fmt.Sprintf("Hello！%s 😊\n您申请的StarRocks的账号流程已经结束。\n%s，%s\n如有其他疑问或需要帮助可加入[StarRocks总群](%s)提问，感谢您使用StarRocks！\n%s",
					username, option, other, util.Read.Schema.GroupURI, message)

				util.Loggrs.Info(client, messages)
				...
				if util.Read.Schema.Lkremind != nil {
					go lark.Send2Markdown(fmt.Sprintf("%s向%s发送了一条信息", time.Now().Format("2006-01-02 15:04:05"), add.Owner), messages, util.Read.Schema.Lkremind)
				}
			}()
