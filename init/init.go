...
func init() {
	...
	// 这是StarRocks连接信息配置表
	contb := "chengken.starrocks_information_connections"
	r := connect.Raw(fmt.Sprintf("select * from %s where status >= 1", contb)).Scan(&util.MetaLink)
	if r.Error != nil {
	...
	//这是基本信息配置表
	conftb := "chengken.starrocks_information_slowconfig"
	r = connect.Raw(fmt.Sprintf("select * from %s ", conftb)).Scan(&util.MetaConf)
	if r.Error != nil {
		fmt.Println(r.Error.Error())
		return
	}
